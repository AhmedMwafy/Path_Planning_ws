#include "../include/graph_search_algorithms/a_star_planner.hpp"
#include <vector>
#include "rmw/qos_profiles.h"
#include <queue>


A_StarPlanner::A_StarPlanner() : Node("dijkstra_planner")
{
    ...
    map_sub_=  create_subscription<nav_msgs::msg::OccupancyGrid>("/map" , map_qos , std::bind(&A_StarPlanner::mapCallback , this , std::placeholders::_1));
    pose_sub_= create_subscription<geometry_msgs::msg::PoseStamped>("/goal_pose" , 10 , std::bind(&A_StarPlanner::goalCallback , this , std::placeholders::_1));
    map_pub_ = create_publisher<nav_msgs::msg::OccupancyGrid>("/a_star/visitedmap" , 10) ;
    path_pub_  = create_publisher<nav_msgs::msg::Path>("/a_star/path" , 10) ;
}



void A_StarPlanner::mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr map){
    if(!map){
...
void A_StarPlanner::goalCallback(const geometry_msgs::msg::PoseStamped::SharedPtr pose){
    if(!pose){
        return ;
    }
    if(!map_){
...
nav_msgs::msg::Path A_StarPlanner::plan(geometry_msgs::msg::Pose & start,  geometry_msgs::msg::Pose & goal){
    ...
    GraphNode Starting_Node = PoseToGrid(start);
    GraphNode Final_Node = PoseToGrid(goal);
    Starting_Node.heuristic = manhattanDistance(Starting_Node,Final_Node);
    std::vector<GraphNode> visited_nodes;
    /* The First pop option is with the least cost -> greater */
    Possible_Options.push(Starting_Node);
    ...
    while(!Possible_Options.empty() && rclcpp::ok()){
        ...
        for(const auto & dir : explore_directions){
            ...
            && map_->data.at(GridToOGIndex(New_Node)) == 0 ){
                ...
                New_Node.heuristic = manhattanDistance(New_Node,Final_Node);
...
GraphNode A_StarPlanner::PoseToGrid (geometry_msgs::msg::Pose & Copy_Pose){
...
int A_StarPlanner::GridToOGIndex(GraphNode & Copy_point){
    return (Copy_point.y * map_->info.width + Copy_point.x ) ;
}
        
bool A_StarPlanner::PoseOnMap(const GraphNode & Copy_Node){
...
geometry_msgs::msg::Pose A_StarPlanner::GridToWorld(const GraphNode & node)
{
...
double A_StarPlanner::manhattanDistance(const GraphNode &node, const GraphNode &goal_node){
    return abs(node.x - goal_node.x) + abs(node.y - goal_node.y);
}

int main(int argc, char **argv) {
  rclcpp::init(argc, argv);
  auto node = std::make_shared<A_StarPlanner>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
